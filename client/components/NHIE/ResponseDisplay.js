...
const ResponseDisplay = props => {
  ...
  const [nickname, nicknameLoading, nicknameError] = useObjectVal(db.ref('players/' + firstID + '/nickname'));
  ...
  const subtract20 = () => {
    db.ref('gameSessions/' + session[0] + '/players/' + uid + '/points').set(session[1].players[uid].points - 20)
  }
